...
public class BoffinsManager {
    ...
    public Map<Placement, Integer> manageEvaluators(Game game, Set<Placement> possiblePlacements) throws InterruptedException {


        // Starten der Kalkulation
        possiblePlacements.forEach(placement -> Work.workToDo.add(new TurnEvaluation(game.lastTurn().getTurnNumber(), game.getBoard().copy(), placement)));
        ...
        Map<Placement, Integer> results = TurnEvaluation.finishedCalculations.stream().flatMap(map -> map.entrySet().stream()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (eval1, eval2) -> eval1));
